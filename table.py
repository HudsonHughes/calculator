...
class Data_Cell(Cell):
	def __init__(self, master, variable, anchor=W, bordercolor=None, borderwidth=1, padx=0, pady=0, background=None,
	             foreground=None, font=None, index=-1, date_cols=[]):
...
class Table(Frame):
	def __init__(self, master, columns, column_weights=None, column_minwidths=None, height=500, minwidth=20,
	             ...
	             scroll_vertically=True, date_cols=[]):
		...
		self._innerborder_width = 8 if innerborder else 0
		...
		for j in range(len(columns)):
			...
			header_cell = Header_Cell(self._head, text=column_name,
	...
	def _append_n_rows(self, n):
		...
		for i in range(number_of_rows, number_of_rows + n):
			...
			for j in range(number_of_columns):
				...
				if len(self._data_colors) > 0:
					bg_color = self._data_colors[i, j]
				else:
					bg_color = self._stripped_rows[i % 2]


				if self._stripped_rows:
					cell = Data_Cell(self._body, borderwidth=self._innerborder_width, variable=var,
						bordercolor=bg_color, padx=self._padx, pady=self._pady,
						background="white", foreground=self._cell_foreground,
						font=self._cell_font, anchor=self._cell_anchor, index=j, date_cols=self._date_cols)
	...
	def set_data(self, data):
		...
		for i in range(n):
			for j in range(m):
				...
				if j in self._date_cols:
					self._data_vars[i][j].set(epoch2human(data[i][j]))


		if self._on_change_data is not None: self._on_change_data()

	def set_colors(self, data):
		self._data_colors = data
